...
main checkout or a sibling worktree — mitigating [claude-code #36182](https://github.com/anthropics/claude-code/issues/36182).
No-ops entirely when you're not in a linked worktree.

## Contents

| Kind | Name | Event | Purpose |
|---|---|---|---|
| skill + command | `worktree` | `/worktree`, or worktree-mode triggers | The **parallel-work protocol**: drive one topic on its own worktree as small reviewed slices, landing to the integration branch only on the human's explicit OK. No captain — the human coordinates; `.agent/worktrees.md` is the board. Covers reserve → plan+example → execute → close, plus pause/resume, recycle, and the self-serializing land loop. |
...
write-guard for its leak protection.
...
merge target (`main`, `develop`, or `trunk`) for `<integration>` wherever it appears. Worktree branches
are cut off it; nothing reaches it except through the human-gated review→land flow. It assumes the
`.agent/` house layout for its board (`.agent/worktrees.md`) and per-topic plans (`.agent/plan/<slug>.md`).
