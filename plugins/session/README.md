# session — Claude Code session continuity

Part of the [`opsi`](../../README.md) marketplace.

Write the handoff at session close; boot the next session from it.

## Contents

| Kind | Name | Purpose |
|---|---|---|
| skill + command | `handoff` | Rewrite `.agent/handoff.md` — ephemeral **≤4k** next-session notes, rich 7-section shape, route durable knowledge out to lessons/docs. |
| skill + command | `continue` | Resume from `.agent/handoff.md` — sanity-check git state, surface pending todos, state a one-line plan, start. |
| skill + command | `session-summary` | Shareable changelog of what a session shipped — themes, decisions, pending. |
| hook | `session-start` | SessionStart — injects a state snapshot (branch · uncommitted · last commit), handoff freshness, todo count, an enforcement-health warning (loud if `python3` is missing → guards disarmed), and an audit-staleness nudge. Fully conditional; safe in any repo. |

Assumes the **`.agent/` house layout** (`.agent/handoff.md`, `.agent/lessons/`, `.docs/`).

_Planned:_ `collab`, a stop-nudge hook.

## Enable

```json
{ "enabledPlugins": { "session@opsi": true } }
```
