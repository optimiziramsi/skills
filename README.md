...
| Plugin | Provides |
|---|---|
| [`git`](plugins/git) | Git **safety net** — a `git-guard` hook that blocks push/pull/fetch, bulk adds, non-FF merges, protected-branch moves, `reset --hard`, `--no-verify`, and discards (rebase/amend pass by default for FF landing flows), regardless of commit style — plus the **hotfix** skill (test-first, cherry-pick both ways, remotes handed to you). |
...
| [`flow`](plugins/flow) | Work management — **plan**, **milestone**, **scope-cut**, **triage-todo**, **feedback** (`.agent/FEEDBACK.md` ledger), plus autonomous background execution: **looper** / **grind** / **collab** skills driven by a shipped Python-3 runner (`bin/loop`, `bin/grind`) that runs jobs as headless `claude` sessions. Ships a `todo-readonly-guard` hook — `.todo` stays user-owned (arm with "ALLOW TODO"). |
...
| [`worktree`](plugins/worktree) | Parallel isolated work — the **worktree** skill (reserve → plan → review-gated slices → land to an integration branch; pause/resume/recycle) + three guards that keep edits inside the active worktree (mitigates [claude-code #36182](https://github.com/anthropics/claude-code/issues/36182)). |
...
- **Commits:** bare imperative single line, automatic by default, opt out with "don't commit".
